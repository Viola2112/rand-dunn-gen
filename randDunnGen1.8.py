...
# Agenda:
    ...
    # Far future:
        ...
        # type in Minecraft (preferrably through GUI)
...
but can be '' if it is the starting room"""
    ...
    if isNotCorridor:
        ...
        width = 30 + 120*random()
        shapeList = ["rectangle"]*2 + ["oval"] * 2 + ["octagon"]
        shape = shapeList[randrange(5)]
        if shape == "octagon":
...
def collectStats(num=100,att=3,showProgress=False): # Work in progress
    ...
    for i in range(num):
        myWin = GraphWin("My Dungeon",1500,750)
        startDoors = shuffle(["N","S","E","W"])
        startRoom = Room(675,300,825,450,"rectangle",startDoors,myWin)
        startRoom.draw()
        myDun = Dungeon([startRoom],myWin)
