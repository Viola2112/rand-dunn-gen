...
# Improvements since Random Dungeon Generator 1.3:
    ...
    # attached to those doors, generate in a randomized
...
# Improvements since Random Dungeon Generator 2.1:
    ...
    # New preset for short corridors
...
# Improvements since Random Dungeon Generator 2.2:
    # Some basic Chaos functions created -- nothing implimented yet

# Improvements since Chaos Dungeon Generator 1.0:
    # Utilizing all chaos funtions!
        # Changed starting value of CHAOS.times from 1 to 10
    # Made it so main() and textMain() utilized seeds if the
    # user wants them to
...
# Agenda:
    ...
    # Allow user to specify chaos seeds, etc.
    # Far future:
        ...
        # Do something like in Processing
            # Maybe even in 3-D!!!
...
######################################################################
# Chaos functions in here

# Do need to not use global variables and pass everyting as parameter???

# Possible solution: create a chaos class w/ variables shared by
...
CHAOS = Chaos() # Global chaos object for use in methods

def reSeedRand(newTimes=10):
    CHAOS.value = random()
    CHAOS.times = newTimes

def reSeedCustom(newValue,newTimes=10):
    CHAOS.value = newValue
    CHAOS.times = newTimes

def nextChaos():
    for i in range(CHAOS.times):
        temp = CHAOS.value
        CHAOS.value = 3.9*temp*(1-temp)
    return CHAOS.value

def chaosBool(weight = 0.5):
    x = nextChaos()
    if x < weight:
        return 0
    else:
...
def chaosRange(stop):
    #Returns in int in [0,stop), like randRange
    x = nextChaos()
    result = int(stop*x)
    if result == stop:
        x = x - 1
...
,'octagon', or 'oval'. doorDirections should be a list of the characters 'N', 'E',
...
but can be '' if it is the starting room"""
    ...
    isNotCorridor = chaosBool(shapeSeed[0]) 
    if isNotCorridor:
        length = sizeSeed[0] + sizeSeed[1]*nextChaos()
        width = sizeSeed[2] + sizeSeed[3]*nextChaos()
        shapeList = (["rectangle"]*shapeSeed[1] + ["oval"]*shapeSeed[2] +
                     ["octagon"]*shapeSeed[3])
        shape = shapeList[chaosRange(len(shapeList))]
        if shape == "octagon":
        ...
        if chaosBool(): # E/W or N/S
            length = sizeSeed[4]
            width = sizeSeed[5]+sizeSeed[6]*nextChaos()
        else:
            length = sizeSeed[5]+sizeSeed[6]*nextChaos()
            width = sizeSeed[4]
        shape = "rectangle"
        spec = ""
    doorList = [startDoor]
    if startDoor == "":
        doorList = []
    for door in ["N","S","E","W"]:
        if (door != startDoor) and (chaosBool(0.25)):
...
def main(num=1,att=3,pause=0,seedList=SEED_LIST,chaosSeed=False):
    """Note: chaosSeed is either False or the Seed value"""
    if chaosSeed > 0:
        reSeedCustom(chaosSeed)
    if num > 100 or att > 100 or (num*att)>1000 or chaosSeed >= 1:
...
def printIntro():
    print("Welcome to Chaos Dungeon Generator",VERSION,"!")
    print("This program generates psuedo-random dungeons for")
...
def getSeed():
    ...
    print("Would you like to use the standard terrain")
    standard = input("seed for dungeon generation? ")
...
def getChaos():
    print("Would you like to use a custom chaos")
    chaos = input("seed for dungeon generation? ")
    if chaos[0] == "y" or chaos[0] == "Y":
        return eval(input("Enter a seed value on the interval (0,1): "))
    else:
        return random()
