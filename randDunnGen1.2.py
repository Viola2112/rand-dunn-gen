...
# Next on agenda: make a new random generator
# file which allows user to customize certain
# variables, such as pool frequency, shape of
# rooms frequencies, etc, much like customized
# world type in Minecraft.

# Also: add octagonal rooms, and mode in which you can see rooms
...
but can be '' if it is the starting room"""
    ...
    if isNotCorridor:
        ...
        specList = ["","pool"] #changing this to underwater for testing
        spec = specList[randBool(0.8)]
        if spec == "pool" and randBool(0.8): # same
            if randBool():
                spec = "underwater"
            else:
                if randBool(0.2):
